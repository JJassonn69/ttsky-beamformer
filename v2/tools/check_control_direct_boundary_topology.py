#!/usr/bin/env python3
"""Audit all nine extracted direct-boundary nets and preserved supplies."""
...
def audit(
    spice: str, mapping: dict[str, Any], routes: list[dict[str, Any]], log: str
) -> dict[str, Any]:
    errors: list[str] = []
    ...
    for route in sorted(routes, key=lambda item: item["net"]):
        ...
        attached = attachments.get(net, [])
        actual = Counter(
            (cell, pin) for instance, cell, pin in attached
            if instance.startswith("Xsky130_fd_sc_hd__") and pin not in POWER_EXPECTED
        )
        helpers = sorted(
            f"{instance}.{pin}" for instance, _cell, pin in attached
        ...
        driver_count = sum(count for role, count in actual.items() if role in driver_roles)
        checks.append({
            "net": net,
        ...
        if net not in attachments:
            ...
            independent.append(net)
        if actual != expected:
            errors.append(f"{net}: mapped roles {actual} != {expected}")
        if helpers:
            errors.append(f"{net}: unexpectedly reaches analog helper roles {helpers}")
        if driver_count != 1:
            errors.append(f"{net}: extracted {driver_count} mapped output drivers")
    if len(set(independent)) != len(routes):
        errors.append("direct-boundary routes are not nine independent named extracted nets")
    ...
    if Counter(item[-1] for item in mapped_instances) != Counter(expected_mapped_types):
        errors.append("mapped standard-cell counts changed in direct-boundary extraction")
    ...
    if fatal:
        errors.append(f"Magic direct-boundary extraction log has fatal markers: {fatal}")
    for marker in (
        "CONTROL_DIRECT_EXTRACTION_DRC_COUNT=0",
        "CONTROL_DIRECT_EXTRACTION_FEEDBACK_COUNT=0",
        "CONTROL_DIRECT_HIER_SPICE=",
        "CONTROL_DIRECT_FLAT_SPICE=",
    ):
        if marker not in log:
            errors.append(f"Magic direct-boundary extraction log lacks {marker}")
    if log.count("exttospice finished.") < 2:
        errors.append("both direct-boundary topology views were not written")
    return {
        "status": "pass" if not errors else "fail", "errors": errors,
        "direct_boundary_role_checks": checks,
        "verified_direct_boundary_net_count": len(checks),
...
def main() -> None:
    ...
    parser.add_argument("--spice", type=Path, default=Path("build/v2/control_routing/direct_boundary_extraction/control_direct_hier.spice"))
    ...
    parser.add_argument("--log", type=Path, default=Path("build/v2/control_routing/direct/direct_magic_extraction.log"))
    parser.add_argument("--report", type=Path, default=Path("build/v2/control_routing/direct_boundary_extraction/topology_audit.json"))
    args = parser.parse_args()
    allocation = json.loads(args.allocation.read_text())
    report = audit(
        args.spice.read_text(errors="replace"), json.loads(args.mapping.read_text()),
        [item for item in allocation["nets"] if item["class"] == "direct_boundary"],
