...
def validate(
    ...
    if trim["channel_order"] != [0, 1, 2, 3]:
        errors.append(
            "trim tracks must follow logical active_trim_codes channel order 0,1,2,3"
        )
    if trim["bit_order_within_channel"] != [0, 1, 2, 3]:
        errors.append(
            "trim tracks must follow logical active_trim_codes bit order 0,1,2,3"
        )
