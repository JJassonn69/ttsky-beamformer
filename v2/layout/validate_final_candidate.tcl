...
set MARKER CONTROL_QUADRATURE
if {[info exists ::env(BF_FINAL_GDS)]} {
    set INPUT_GDS [file normalize $::env(BF_FINAL_GDS)]
}
if {[info exists ::env(BF_FINAL_WORKDIR)]} {
    set WORKDIR [file normalize $::env(BF_FINAL_WORKDIR)]
}
if {[info exists ::env(BF_FINAL_TOP)]} {
    set TOP $::env(BF_FINAL_TOP)
}
if {[info exists ::env(BF_FINAL_MARKER)]} {
    set MARKER $::env(BF_FINAL_MARKER)
}
file mkdir $WORKDIR
...
puts "${MARKER}_EXTRACTION_DRC_COUNT=$drc_count"
puts "${MARKER}_IMPORT_FEEDBACK_COUNT=$import_feedback"
...
puts "${MARKER}_EXTRACTION_FEEDBACK_COUNT=$extract_feedback"
...
puts "${MARKER}_HIER_SPICE=$HIER"
puts "${MARKER}_FLAT_SPICE=$FLAT"
quit -noprompt
