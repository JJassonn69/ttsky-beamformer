# Read back and extract the exact direct-GDS V3 controller/boundary handoffs.
set PROJECT_ROOT [file normalize [pwd]]
if {[info exists ::env(V3_CONTROL_BOUNDARY_GDS)]} {
    set INPUT_GDS [file normalize $::env(V3_CONTROL_BOUNDARY_GDS)]
} else {
    set INPUT_GDS [file join $PROJECT_ROOT build v3 control_boundary_handoffs direct v3_four_channel_ctrl_boundary_handoffs.gds]
}
if {[info exists ::env(V3_CONTROL_BOUNDARY_READBACK_DIR)]} {
    set WORKDIR [file normalize $::env(V3_CONTROL_BOUNDARY_READBACK_DIR)]
} else {
    set WORKDIR [file join $PROJECT_ROOT build v3 control_boundary_handoffs magic_readback]
}
set TOP v3_four_ch_ctrl_boundary_handoff
file mkdir $WORKDIR
cd $WORKDIR
...
if {$gds_feedback != 0} {
    ...
    error "V3 controller/boundary handoff GDS produced import feedback"
}
load $TOP
select top cell
expand
...
if {$drc_count != 0} {
    ...
    error "V3 controller/boundary handoff GDS has Magic DRC errors"
}
