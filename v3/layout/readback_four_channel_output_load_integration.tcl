# Read back the exact V3 output-load and analog-pad integration.
...
set WORKDIR [file join $PROJECT_ROOT build v3 four_channel_output_load_integration readback]
set TOP v3_four_channel_output_load_integration
file mkdir $WORKDIR
cd $WORKDIR
...
puts "V3_FOUR_CHANNEL_OUTPUT_LOAD_DRC_COUNT=$drc_count"
if {$drc_count != 0} {
    feedback save [file join $WORKDIR drc_feedback.txt]
    error "V3 output-load integration has Magic DRC errors"
}
extract unique notopports
extract do local
extract all
set extraction_feedback [feedback count]
puts "V3_FOUR_CHANNEL_OUTPUT_LOAD_EXTRACTION_FEEDBACK_COUNT=$extraction_feedback"
...
puts "V3_FOUR_CHANNEL_OUTPUT_LOAD_FEEDBACK_AFTER_CLEAR=[feedback count]"
quit -noprompt
