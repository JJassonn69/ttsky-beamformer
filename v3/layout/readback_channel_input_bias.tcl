# Read back the final URPM-repaired GDS and re-run Magic topology checks.
...
set WORKDIR [file join $PROJECT_ROOT build v3 channel_input_bias_readback]
...
puts "V3_CHANNEL_INPUT_BIAS_READBACK_DRC_COUNT=$drc_count"
if {$drc_count != 0} {
    feedback save [file join $WORKDIR drc_feedback.txt]
    error "final input-bias GDS readback has DRC errors"
}
extract unique notopports
extract do local
extract all
set extraction_feedback [feedback count]
puts "V3_CHANNEL_INPUT_BIAS_READBACK_EXTRACTION_FEEDBACK_COUNT=$extraction_feedback"
...
puts "V3_CHANNEL_INPUT_BIAS_READBACK_FEEDBACK_AFTER_CLEAR=[feedback count]"
quit -noprompt
