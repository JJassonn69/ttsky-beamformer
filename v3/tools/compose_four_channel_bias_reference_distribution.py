"""Draw the balanced shared VBIAS and REF trees using KLayout Python."""
...
manifest = json.loads((root / "v3/layout/four_channel_bias_reference_distribution.json").read_text())
source = root / manifest["source"]["gds"]
if hashlib.sha256(source.read_bytes()).hexdigest() != manifest["source"]["gds_sha256"]:
    raise RuntimeError("refusing support-tree composition on a non-gated source GDS")

layout = pya.Layout()
layout.read(str(source))
source_top = layout.cell(manifest["source"]["top_cell"])
if source_top is None:
    raise RuntimeError("phase-distribution source top is missing")
...
def route(record):
    ...
    if x1 != x2 and y1 != y2:
        raise RuntimeError("non-Manhattan support-tree segment")
...
for tree in manifest["trees"].values():
    ...
    top.shapes(layers["metal3_label"]).insert(
