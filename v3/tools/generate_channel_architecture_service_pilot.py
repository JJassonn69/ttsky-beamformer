#!/usr/bin/env python3
"""Generate all 15 exact units plus the selected output/analog service corridors."""
...
TOP = "v3_channel_architecture_service"
SPINE_BOTTOM_Y = 5.60
SPINE_TOP_Y = 108.80
...
def build_tcl(
    ...
    source = matrix_gen.build_tcl(
        matrix, unit, catalog, ground_bus_offset_um=0.20,
    )
    source = source.replace("channel_matrix_pilot", "channel_architecture_service")
    source = source.replace("v3_channel_matrix_pilot", TOP)
    source = source.replace("V3_CHANNEL_MATRIX", "V3_CHANNEL_ARCHITECTURE_SERVICE")
    source = source.replace("channel-matrix pilot", "channel-architecture service pilot")

    alias = re.compile(
        r"box [^\n]+\n"
        r"label u\d{2}_(?:sig|ref|vbias|outp|outn) center (?:metal2|metal3|metal4)\n"
    ...
    selected_architecture = architecture["candidates"][selected_name]
    rows = {
        int(item["row_top_to_bottom"]): item
        for item in selected_architecture["row_track_plan"]
    }
    by_row: dict[int, list[dict[str, Any]]] = {}
    for item in matrix["matrix"]["instances"]:
        by_row.setdefault(int(item["row_top_to_bottom"]), []).append(item)
    output_spines = selected_architecture["service_corridors"]["output_left"]["spines"]
    analog_spines = selected_architecture["service_corridors"]["analog_right"]["spines"]
    commands: list[str] = []
    ...
    for row, instances in sorted(by_row.items()):
        ...
        for role, width in (("sig", 0.40), ("vbias", 0.40), ("ref", 0.30)):
            ports = sorted(
                [item["ports"][role]["point"] for item in instances],
                key=lambda point: point[0],
            )
            level = analog_levels[role]
            for port in ports:
                commands.append(wire_v("metal4", port[0], level, port[1], width))
                commands.extend(via3([port[0], level]))
            spine_x = float(analog_spines[role])
            commands.append(wire_h(
                "metal3", min(spine_x, ports[0][0]), max(spine_x, ports[-1][0]),
                level, width,
            ))
            commands.extend(via2([spine_x, level]))

    commands.extend([
        wire_v("metal2", float(output_spines["p"]), SPINE_BOTTOM_Y, SPINE_TOP_Y, 0.50),
        wire_v("metal2", float(output_spines["n"]), SPINE_BOTTOM_Y, SPINE_TOP_Y, 0.50),
        wire_v("metal2", float(analog_spines["sig"]), SPINE_BOTTOM_Y, SPINE_TOP_Y, 0.40),
        wire_v("metal2", float(analog_spines["vbias"]), SPINE_BOTTOM_Y, SPINE_TOP_Y, 0.40),
        wire_v("metal2", float(analog_spines["ref"]), SPINE_BOTTOM_Y, SPINE_TOP_Y, 0.30),
    ])
    labels = [
        matrix_gen.label("row_outp", [float(output_spines["p"]), SPINE_TOP_Y], "metal2", output=True),
        matrix_gen.label("row_outn", [float(output_spines["n"]), SPINE_TOP_Y], "metal2", output=True),
        matrix_gen.label("sig", [float(analog_spines["sig"]), SPINE_TOP_Y], "metal2"),
        matrix_gen.label("vbias", [float(analog_spines["vbias"]), SPINE_TOP_Y], "metal2"),
        matrix_gen.label("ref", [float(analog_spines["ref"]), SPINE_TOP_Y], "metal2"),
    ]
    insertion = "\n".join(commands + labels)
    marker = "\nsave $TOP.mag\nwriteall force\n"
    if source.count(marker) != 1:
        raise RuntimeError("matrix service insertion marker changed")
    source = source.replace(marker, f"\n{insertion}{marker}")

    final_marker = "puts \"V3_CHANNEL_ARCHITECTURE_SERVICE_GDS_FEEDBACK_COUNT=[feedback count]\"\nquit -noprompt"
    final_replacement = """set gds_feedback [feedback count]
puts \"V3_CHANNEL_ARCHITECTURE_SERVICE_GDS_FEEDBACK_COUNT=$gds_feedback\"
if {$gds_feedback != 0} {
    feedback save [file join $WORKDIR gds_feedback.txt]
}
set summary [open [file join $WORKDIR physical_markers.txt] w]
puts $summary \"V3_CHANNEL_ARCHITECTURE_SERVICE_DRC_COUNT=$drc_count\"
puts $summary \"V3_CHANNEL_ARCHITECTURE_SERVICE_EXTRACTION_FEEDBACK_COUNT=$extraction_feedback\"
puts $summary \"V3_CHANNEL_ARCHITECTURE_SERVICE_GDS_FEEDBACK_COUNT=$gds_feedback\"
close $summary
quit -noprompt"""
    if source.count(final_marker) != 1:
        raise RuntimeError("matrix service final marker changed")
